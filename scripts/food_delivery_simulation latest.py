...
day = np.random.randint(1, 31, size=N_ORDERS)          
hour_of_day = np.random.randint(0, 24, size=N_ORDERS) 
is_peak_hour = np.isin(hour_of_day, list(PEAK_HOURS))
item_count = np.random.randint(1, 9, size=N_ORDERS)   
...
orders_out["is_peak_hour"] = orders_out["is_peak_hour"].astype(int)  
